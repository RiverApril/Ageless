...
namespace Ageless {
	...
	public class Game {
		...
		bool intercectTriangleRay(Vector3 V1, Vector3 V2, Vector3 V3, Vector3 O, Vector3 D) {
		...
		void onMouseDown(object sender, MouseButtonEventArgs e) {
			switch (e.Button) {
				case MouseButton.Left: {
					...
					float normX = (gameWindow.Mouse.X - (gameWindow.Width / 2.0f)) / (gameWindow.Width / 2.0f);
					float normY = (gameWindow.Height - gameWindow.Mouse.Y - (gameWindow.Height / 2.0f)) / (gameWindow.Height / 2.0f);
					...
					if (options.Count > 0) {
						...
						float lenSq = FAR * FAR;
						Vector3 close = options[0];
	
						foreach (Vector3 option in options) {
							float l = (option - origin).LengthSquared;
							if (l < lenSq) {
								close = option;
							}
						}
	
						//Console.WriteLine("Bingo! : {0}, {1}, {2}", close.X, close.Y, close.Z);
						player.target = new Vector3(close.X, loadedWorld.getFloorAtPosition(close.X, close.Y + player.maxSlope, close.Z), close.Z);
					}
